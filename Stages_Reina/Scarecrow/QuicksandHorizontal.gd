...
export(float,1,30) var rising_speed=1
export(int,0,32) var draw_x_offset=0
...
func _draw():
	...
	draw_texture_rect_region(bottom[j],
		#x,y,width,height
		Rect2(-drawSize.x,-drawSize.y,drawSize.x*2,drawSize.y*2+sandFallProgress*2),
		Rect2(16+8,draw_x_offset,drawSize.x/2,drawSize.y/2+sandFallProgress/2) #I have no idea how this works
	)
	
	draw_texture_rect_region(sandFallSpr,
		Rect2(-drawSize.x,-drawSize.y-8,drawSize.x*2,8),
		Rect2(0,4*j,drawSize.x/2,2)
	)
	draw_set_transform(Vector2(0,0),PI,Vector2(1,1))
	draw_texture_rect_region(sandFallSpr,
		Rect2(-drawSize.x,-drawSize.y-8-sandFallProgress*2,drawSize.x*2,8),
