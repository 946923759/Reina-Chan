...
#We need to support expressions so
#var portrait_textures=[]
#var cur_expression:int=0
var cur_texture:Texture

var mask1 = preload("res://Screens/ScreenCutscene/maskBox.png")
var mask2 = preload("res://Screens/ScreenCutscene/maskBox2.png")
...
func set_texture_wrapper(sprName):
	...
	var f = File.new()
	if OS.has_feature("standalone") and !f.file_exists("res://Screens/ScreenCutscene/Portraits/"+sprName+".png.import"):
		...
		cur_texture=load("res://Screens/ScreenCutscene/Portraits/"+sprName+".png")
	update()

func overdrive_set_texture(database:Dictionary,portraitKey:String):
	lastLoaded=portraitKey
	if portraitKey in database:
		var toLoad = database[portraitKey]
		for i in range(toLoad):
			var sprName = toLoad[i]
			#set_texture(load("res://Cutscene/Portraits/"+sprName+".png"))
			var f = File.new()
			if OS.has_feature("standalone") and !f.file_exists("res://Screens/Cutscene/Portraits/"+sprName+".png.import"):
				var path = OS.get_executable_path().get_base_dir()+"/GameData/Cutscene/Portraits/"+sprName+".png"
				#print("Checking path "+path)
				if f.file_exists(path):
					print("Found external image file at "+path)
				...
				cur_texture=load("res://Screens/Cutscene/Portraits/"+sprName+".png")
		update()
