...
func _ready():
	...
	for i in range(size):
		...
		if i < size-1:
			...
			tw.tween_property(top,"rect_scale:y",2.0,.3).from(0.0).set_delay(endTime).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN)
			tw.tween_property(bottom,"rect_scale:y",2.0,.3).from(0.0).set_delay(endTime).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN)
...
func _process(delta):
	...
	if total>=seq[step][0]:
		...
		if step<size-1:
			...
			newTw.set_parallel()
			newTw.tween_property(top,"rect_scale:y",0.0,.3).from(2.0).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
			newTw.tween_property(bottom,"rect_scale:y",0.0,.3).from(2.0).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
